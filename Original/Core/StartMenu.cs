...
namespace SamplyGame
{
    ...
    public class StartMenu : Component
	{
		...
		protected override async void OnUpdate(float timeStep)
		{
            ...
            if (Ethereum.GameScoreService.Current != null)
            {
                if (string.IsNullOrEmpty(topPlayers.Value))
                {

                    await Application.InvokeOnMainAsync(() =>
                    {
                        var topScores = Ethereum.GameScoreService.Current.GetTopScoresAsync().Result;
                        var text = "\n\nTOP PLAYERS\n\n";
                        foreach (var score in topScores)
                        {
                            text = text + score.Address.Substring(0, 15) + "...  " + score.Score + "\n";
                        }
                        topPlayers.Value = text;
                    });
                }
